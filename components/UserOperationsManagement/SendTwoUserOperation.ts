...
export async function sendTwoUserOperationWithPaymaster(to: string, amount: string, erc20TokenAddress: string, erc20ReceiverAccountAddress: string, erc20TokenAmount: string, accountIdentifier: string, sessionOwnerPrivateKey: string, sessionAllowedSmartContracts: string[], sessionAllowedToAddresses: string[]){
    ...
    const transaction1= {
        ...
        const { proof, publicSignals } = await snarkjs.groth16.fullProve(circuitInputs, "zk_session_key_two_transaction_validation.wasm", "zk_session_key_two_transaction_validation_0001.zkey");
